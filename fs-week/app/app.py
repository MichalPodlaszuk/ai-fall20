...
""".strip()
    ...
    def create_user(self, email, password):
        ...
        df = pandas_select('SELECT * FROM users')
        with self.conn as c:
            c.execute(
...
def user_create():
    ...
    if request.method == "POST":
        ...
        df = pandas_select(sentence)
        if email and (email not in list(df)) and password:
            db.create_user(email=email, password=password)

            return "ok"
        elif email in df:
            return "This email is already occupied. Please use another one."
...
def post_image():
    ...
    if request.method == "POST":
        ...
        decision = request.form['prediction']
        ...
        # return r.json()
            ...
            r = requests.post("http://127.0.0.1:5001/predict_correct", files={"file": img_bytes})
